...
# IMPORTANT: Add these to your .streamlit/secrets.toml file!
# [supabase]
...
def clerk_dashboard():
    ...
    with st.form("clerk_entry_form"):
        ...
        with col1:
            ...
            branch = st.selectbox("Branch", ["CSE", "ISE", "ECE", "MECH", "CIVIL", "EEE", "AI & ML"])
        ...
        with col3:
            ...
            college_account = st.selectbox("Credited To A/C", ["SBI - Main (Ending 1234)", "HDFC - Fees (Ending 5678)", "SBI - Exam (Ending 9012)"])
            
        st.markdown("---")
        submitted = st.form_submit_button("💾 Save & Sync Receipt", type="primary", use_container_width=True)
        
        if submitted:
            if not usn or not student_name or not utr:
                st.error("⚠️ USN, Name, and UTR are mandatory fields!")
                ...
                data = {
                    ...
                    "college_account": college_account,
                ...
                try:
                    ...
                    st.success(f"✅ Receipt for ₹{amount} (UTR: {utr}) saved successfully!")
                except Exception as e:
                    # Catch duplicate UTRs based on our SQL unique constraint
                    if "duplicate key value" in str(e).lower():
                        st.error(f"❌ Duplicate Entry! The UTR {utr} has already been entered in the system.")
                    else:
                        st.error(f"Database error: {e}")
...
def admin_dashboard():
    ...
    with tab1:
        ...
        if st.button("Fetch Data"):
            try:
                # Fetch data from Supabase within date range
                ...
                if not df.empty:
                    # Reorder columns for the export
